...

{
	if(_x in life_market_shortnames) then {
		_didanything = true;
		_row = [_x] call noaim_fnc_marketGetPriceRow;
		_price = _row select 1;
		_globalchange = _row select 2;
		_lastchange = _row select 3;
		_resname = [_x, 0] call noaim_fnc_varHandle;
		_resname = [_resname] call noaim_fnc_varToStr;
		_text = format["%1<br/><t size='1.2'>%2</t><br/><t size='1.3' color='#FFFF00'>€%3</t><br/>", _text,_resname, [_price] call noaim_fnc_numberText];
