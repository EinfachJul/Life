...
*/
private["_shortname","_amount","_price","_modifier","_globalprice","_dir","_israw","_arr_resource","_arr_price","_modifier","_send","_relamount","_max","_min"];
...
_arr_resource = [_shortname] call noaim_fnc_marketGetRow;
_arr_price = [_shortname] call noaim_fnc_marketGetPriceRow;
...
[_shortname, [_shortname,_price,_globalprice,-(_modifier)], false ] call noaim_fnc_marketSetPriceRow; //dont broadcast!
//[[_shortname,_price],"DB_fnc_marketInsert",false,false] call noaim_fnc_LAST;
[_shortname,_price] remoteExec ["DB_fnc_marketInsert", 2];
...
{
	{
		if((count _x) == 2) then
		{
		    ...
		    {
			[_x select 0, _relamount, true, false] call noaim_fnc_marketBuy; //Make prices higher, no broadcast!
		    }
		    else
		    {
			_relamount = -(_relamount);
			[_x select 0, _relamount, true, false] call noaim_fnc_marketSell; //Make prices higher, no broadcast!
		    };
		}
		else
		{
