...
_value = 0;
{
	...
	if(_index != -1) then {
		...
		if(_vIndex != -1) then {
			...
			_houseInvVal = _houseInvVal - (([_var] call noaim_fnc_itemWeight) * _val);
...
if(_value > 0) then {
	//[[0,"STR_House_Raid_Successful",true,[[_value] call noaim_fnc_numberText]],"noaim_fnc_broadcast",true,false] spawn noaim_fnc_DONE;
	//format["STR_House_Raid_Successful",[(_value)] call noaim_fnc_numberText] remoteExec ["systemChat",-2];
	[0,"STR_House_Raid_Successful",true,[[_value] call noaim_fnc_numberText]] remoteExecCall ["noaim_fnc_broadcast",-2];
	...
	//[[_house],"TON_fnc_updateHouseTrunk",false,false] spawn noaim_fnc_DONE;
} else {
