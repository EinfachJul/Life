...
switch ( _NextCard ) do
{
	case 3: { _Pcard3 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)]; _P3T = format["%1",_number];_p3value ctrlSetText format["%1",_number];};
	case 4: { _Pcard4 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)]; _P4T = format["%1",_number];_p4value ctrlSetText format["%1",_number];};
	case 5: { _Pcard5 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)]; _P5T = format["%1",_number];_p5value ctrlSetText format["%1",_number];};
	default { _Pcard3 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)]; _P3T = format["%1",_number];_p3value ctrlSetText format["%1",_number];}
};
...
if( _gameover ) then
{
	...
	{
		_cash = _betamt * 2;
		[_cash,0,0] call noaim_fnc_handleMoney;
		_winnings ctrlSetText  format["You win by getting BlackJack! You earned $%1",[_cash] call noaim_fnc_numberText];
	}
	else
	{
