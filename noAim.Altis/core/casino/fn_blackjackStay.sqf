...
if( !_gameover ) then
{
	...
	_Dcard3 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
...
if( !_gameover ) then
{
	...
	_Dcard4 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
...
if(_win == 1) then
{
	...
	_winnings ctrlSetText format["You tied. You got your bet back: $%1",[_cash] call noaim_fnc_numberText];
};
...
if(_win ==2) then
{
	_cash = _betamt * 2;
	[_cash,0,0] call noaim_fnc_handleMoney;
	_winnings ctrlSetText format["You won. You earned $%1",[_cash] call noaim_fnc_numberText];
};
