...
life_current_bet = _betamt;
disableSerialization;
_display = findDisplay 5980;

if(life_znorak < _betamt) exitWith {hint format["You don't have enough money to play ($%1)",[_betamt] call noaim_fnc_numberText];};

[_betamt,0,1] call noaim_fnc_handleMoney;
...
_Pcard1 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
...
_Dcard1 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
...
_Pcard2 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
...
{
	_win = 4 * _betamt;
	[_win,0,0] call noaim_fnc_handleMoney;
	_winnings ctrlSetText format["You got BlackJack. You earned $%1",[_win] call noaim_fnc_numberText];
...
else
{
	...
	if( format["%1",_number] == "A" ) then
	{
		...
		{
			_Dcard2 ctrlSetText format["textures\casino\cards\%1_%2.jpg",_number,(ceil(random 3) + 1)];
			_winnings = _display displayCtrl 6001;
		...
		else
		{
			...
			_Dcard2 ctrlSetText "textures\casino\cards\back.jpg";
