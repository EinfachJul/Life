...
if(life_znorak < 1000) exitWith {hint localize "STR_Serive_Chopper_NotEnough"};
life_znorak = life_znorak - 1000;
...
{
	[(_search select 0),1] remoteExec ["noaim_fnc_setFuel",(_search select 0)];
	//[[(_search select 0),1],"noaim_fnc_setFuel",(_search select 0),false] spawn noaim_fnc_DONE;
}
else
{
