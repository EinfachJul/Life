...
_itemName = [([_itemInfo select 0,0] call noaim_fnc_varHandle)] call noaim_fnc_varToStr;
_illegal = [_itemInfo select 0,life_illegal_items] call TON_fnc_index;
if(playerSide == west && _illegal != -1) exitWith
{
	titleText[format[localize "STR_NOTF_PickedEvidence",_itemName,[(life_illegal_items select _illegal) select 1] call noaim_fnc_numberText],"PLAIN"];
	life_zgazzy = life_zgazzy + ((life_illegal_items select _illegal) select 1);
	deleteVehicle _obj;
	life_action_delay = time;
};
life_action_delay = time;
_diff = [_itemInfo select 0,_itemInfo select 1,life_carryWeight,life_maxWeight] call noaim_fnc_calWeightDiff;
if(_diff <= 0) exitWith {hint localize "STR_NOTF_InvFull"};
_obj setVariable["PickedUp",TRUE,TRUE];
if(_diff != _itemInfo select 1) then
{
	if(([true,_itemInfo select 0,_diff] call noaim_fnc_handleInv)) then
	{
...
{
	if(([true,_itemInfo select 0,_itemInfo select 1] call noaim_fnc_handleInv)) then
	{
		deleteVehicle _obj;
		player playmove "AinvPknlMstpSlayWrflDnon";
		sleep 0.5;
