...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship") || (_vehicle isKindOf "Armored") || (_vehicle isKindOf "ReammoBox_F") || (_vehicle isKindOf "StaticWeapon") || (_vehicle isKindOf "Cargo_base_F")) then
{
	...
	_data = missionNamespace getVariable "ImpoundPerm_Prof";
	...
	if((count crew _vehicle) == 0) then
	{
		...
		//[[_vehicle,true,player,true],"TON_fnc_vehicleStore",false,false] spawn noaim_fnc_DONE;
		[_vehicle,true,player,true] remoteExec ["TON_fnc_vehicleStore",2];
		...
		if(_uid == _owner) then {
			...
			//[[0,"STR_NOTF_HasImpounded",true,[profileName,(_vehicleData select 0) select 1,_vehicleName]],"noaim_fnc_broadcast",true,false] spawn noaim_fnc_TRY;
			...
			//[[0,format["Sie erhalten %1 XP und einen Bonus von %2 fuer das Beschlagnahmen", _addExpAmount, [_valBon] call noaim_fnc_numberText]],"noaim_fnc_broadcast",player,false] spawn noaim_fnc_TRY;
			[0,format["Sie erhalten %1 XP und einen Bonus von %2 fuer das Beschlagnahmen", _addExpAmount, [_valBon] call noaim_fnc_numberText]] remoteExecCall ["noaim_fnc_broadcast",player];
			["ImpoundPerm_Prof",_addExpAmount] call noaim_fnc_addExp;
			life_zgazzy = life_zgazzy + _price + _valBon;
		};
	}
	else
	{
