...
[_className] call noaim_fnc_vehicleShop3DPreview;
...
[_basePrice] call noaim_fnc_numberText,
[round(_basePrice * 1.5)] call noaim_fnc_numberText,
...
lbClear _ctrl;
_colorArray = [_className] call noaim_fnc_vehicleColorCfg;

for "_i" from 0 to count(_colorArray)-1 do {
	if((_colorArray select _i) select 1 == (life_veh_shop select 2)) then {
		_temp = [_className,_i] call noaim_fnc_vehicleColorStr;
