...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart","maf","donator","taxi"] && playerSide != civilian) exitWith {hint localize "STR_Shop_NotaCiv"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
if((_this select 3) == "maf" && !license_civ_mafia) exitWith {hint localize "STR_Shop_NotaMaf"; closeDialog 0;};
if((_this select 3) == "donator" && !license_civ_donator) exitWith {hint localize "STR_Shop_NotaDonator"; closeDialog 0;};
if((_this select 3) == "taxi" && !license_civ_taxi) exitWith {hint localize "STR_Shop_NotaTaxi"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) in ["gsg9"] && playerSide != west) exitWith {hint localize "STR_Shop_NotaCop"; closeDialog 0;};
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith { hint localize "STR_Shop_NotaDive"; closeDialog 0;};
if((_this select 3) in ["medic"] && playerSide != east) exitWith { hint localize "STR_Shop_NotaMedic"; closeDialog 0;};
if((_this select 3) in ["alac"] && playerSide != independent) exitWith {hint "STR_Shop_NotaAlac"; closeDialog 0;};

life_clothing_store = _this select 3;

//License Check?
_var = [life_clothing_store,0] call noaim_fnc_licenseType;
if(_var select 0 != "") then
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {
	hint format[localize "STR_Shop_YouNeed",[_var select 0] call noaim_fnc_varToStr]; 
	closeDialog 0;
...
if(isNil "life_clothesPurchased") exitWith
{
	...
	if(backpack player != "") then
	{
			...
			else
		{
			...
			{
				{
					[_x,true,true] call noaim_fnc_handleItem;
	...
	{
		{[_x,true,false,false,true] call noaim_fnc_handleItem;} foreach life_oldUniformItems;
	};
	...
	if(vest player != "") then
	{
		...
		else
		{
			...
			{
				{[_x,true,false,false,true] call noaim_fnc_handleItem;} foreach life_oldVestItems;
			};
		};
	};
};
...
if((life_clothing_purchase select 4) == -1) then
{
	if(life_oldBackpack != backpack player) then
	{
		if(life_oldBackpack == "") then {removeBackpack player;} else
		{
			removeBackpack player;
			player addBackpack life_oldBackpack;
			{[_x,true,true] call noaim_fnc_handleItem;} foreach life_oldBackpackItems;
		};
	};
};
