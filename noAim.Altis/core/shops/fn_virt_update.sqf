...
_shop_data = [life_shop_type] call noaim_fnc_virt_shops;
ctrlSetText[2403,format["%1", _shop_data select 0]];

{
	_name = [([_x,0] call noaim_fnc_varHandle)] call noaim_fnc_vartostr;
	_index = [_x,__GETC__(buy_array)] call TON_fnc_index;
	if(_index != -1) then
	{
		_price = (__GETC__(buy_array) select _index) select 1;
		////Marktsystem Anfang////
		_marketprice = [_x] call noaim_fnc_marketGetBuyPrice;
		if(_marketprice != -1) then
		{
			_price = _marketprice;
		};
		////Marktsystem Ende////
		_item_list lbAdd format["%1  (€%2)",_name,[_price] call noaim_fnc_numberText];
...
{
	_var = [_x,0] call noaim_fnc_varHandle;
