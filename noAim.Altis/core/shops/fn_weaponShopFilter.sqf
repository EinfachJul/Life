...
switch (_index) do
{
	case 0:
	{
		_config = [_shop] call noaim_fnc_weaponShopCfg;
		{
			_itemInfo = [_x select 0] call noaim_fnc_fetchCfgDetails;//ARYX AUKTIONSHAUS ABFRAGE
	...
	case 1:
	{
		...
		((findDisplay 38400) displayCtrl 38405) ctrlSetText localize "STR_Global_Sell";
		{
			...
			{
				_itemInfo = [_x] call noaim_fnc_fetchCfgDetails;
