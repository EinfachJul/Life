...
{
	case "bruce": {[_selection] call noaim_fnc_clothing_bruce;};
	case "donator": {[_selection] call noaim_fnc_clothing_donator;};
	case "cop": {[_selection] call noaim_fnc_clothing_cop;};
	case "gsg9": {[_selection] call noaim_fnc_clothing_gsg9;};
	case "taxi": {[_selection] call noaim_fnc_clothing_taxi;};
	case "medic": {[_selection] call noaim_fnc_clothing_medic;};
	//case "alac": {[_selection] call noaim_fnc_clothing_alac;};
	case "reb": {[_selection] call noaim_fnc_clothing_reb;};
	case "maf": {[_selection] call noaim_fnc_clothing_maf;};
	case "dive": {[_selection] call noaim_fnc_clothing_dive;};
	case "kart": {[_selection] call noaim_fnc_clothing_kart;};
};

if(count _clothes == 0) exitWith {};
{
	_details = [_x select 0] call noaim_fnc_fetchCfgDetails;
