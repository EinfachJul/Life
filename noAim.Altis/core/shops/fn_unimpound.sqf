...
_veh = zaki_vehicles select parseNumber(_veh);
...
if(_veh distance impound_obj < 50) then
{
	...
	_check = nearestObjects[_pos,["LandVehicle","Air","ReammoBox_F","StaticWeapon","Cargo_base_F"],4];
	if(count _check > 0) exitWith {hint localize "STR_Shop_Unimpound_VehExist";};
	if(_price > life_znorak) then
	{
		_price = _price + 200;

		if(_price > life_zgazzy) exitWith {hint localize "STR_Shop_Unimpound_NotEnough"};
	...
	hint format[localize "STR_Shop_Unimpound_Success",_name,[_price] call noaim_fnc_numberText];
	detach _veh;
...
[] call noaim_fnc_impoundMenu; //Refresh it