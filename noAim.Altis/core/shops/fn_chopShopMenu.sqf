...
_nearVehicles = nearestObjects [getMarkerPos (_this select 3),["Car","Truck"],25]; //25
//[[],"chopperDiscoFix"] call BIS_fnc_MP; blimy

if(side player != civilian) exitWith { hint "Du bist im Dienst!" };
...
_control = ((findDisplay 39400) displayCtrl 39402);
{
	if(alive _x) then {
		...
		/*
		_price = [playerSide,"Car",_className] call noaim_fnc_getVehicleSellPrice;
