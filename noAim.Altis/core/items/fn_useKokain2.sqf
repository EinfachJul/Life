...
if ( life_drugged_cocaine > 2) then
{
	...
	if (_dmg < 1.0) then
	{
		...
		["shortWeak"] spawn noaim_fnc_weaknessEffects;
		player setFatigue 1;
	...
	sleep 5;
	["hardDrugUse"] call noaim_fnc_weaknessEffects;
	titleText["Du erhaeltst 'Superkraefte' für 10 Minuten! Achte auf deinen Hunger!","PLAIN"];
	...
	while{life_drugged_cocaine < 1 && Alive player} do 
	{
		...
		if(_count < 1) then {hint "Du bist nun erschoepft und geschwaecht aufgrund einer Überdosis! Rauch einen Joint oder nimm mehr um es zu stoppen!";};
