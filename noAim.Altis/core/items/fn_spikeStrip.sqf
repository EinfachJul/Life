...
life_action_spikeStripPickup = player addAction[localize "STR_ISTR_Spike_Pack",noaim_fnc_packupSpikes,"",0,false,false,"",
' _spikes = nearestObjects[getPos player,["Land_Razorwire_F"],8] select 0; !isNil "_spikes" && !isNil {(_spikes getVariable "item")}'];
//[[_spikeStrip],"TON_fnc_spikeStrip",false,false] spawn noaim_fnc_DONE; //Send it to the server for monitoring.
