...
if(count _itemArray == 0) exitWith
{
    switch(playerSide) do {
        case west: {
            [] call noaim_fnc_copLoadout;
        };
        
        case civilian: {
            [] call noaim_fnc_civLoadout;
        };

        case east: {
            [] call noaim_fnc_medicLoadout;
        };
		
		/*case independent: {
            [] call noaim_fnc_alacLoadout;
        };*/
    };
};
...
if(_goggles != "") then {_handle = [_goggles,true,false,false,false] spawn noaim_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_headgear != "") then {_handle = [_headgear,true,false,false,false] spawn noaim_fnc_handleItem; waitUntil {scriptDone _handle};};
...
life_maxWeight = 100;
{
    _item = [_x,1] call noaim_fnc_varHandle;
    [true,_item,1] call noaim_fnc_handleInv;
} foreach (_yItems);
life_maxWeight = 40;

//Primary & Secondary (Handgun) should be added last as magazines do not automatically load into the gun.
if(_prim != "") then {_handle = [_prim,true,false,false,false] spawn noaim_fnc_handleItem; waitUntil {scriptDone _handle};};
if(_seco != "") then {_handle = [_seco,true,false,false,false] spawn noaim_fnc_handleItem; waitUntil {scriptDone _handle};};
