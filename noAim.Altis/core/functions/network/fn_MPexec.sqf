/*
	...
	Parameter(s):
	_this select 0: STRING - Packet variable name (always "noaim_fnc_MP_packet")
	_this select 1: ARRAY - Packet value (sent by noaim_fnc_LAST function; see it's description for more details)
...
if(!(["noaim_fnc_",_functionName] call BIS_fnc_inString) && {!(["SPY_fnc_",_functionName] call BIS_fnc_inString)} && {!(["DB_fnc_",_functionName] call BIS_fnc_inString)} && {!(["TON_fnc_",_functionName] call BIS_fnc_inString)} &&
{!(toLower(_functionName) in ["bis_fnc_execvm","bis_fnc_effectkilledairdestruction","bis_fnc_effectkilledairdestructionstage2"])} && {!(["SAX_fnc_",_functionName] call BIS_fnc_inString)}) exitWith {false};
if(toLower(_functionName) == "DB_fnc_bsynoCall") exitWith {false};
...
if (ismultiplayer && _mode == 0) then {
	if (isserver) then {
		if (typename _target == typename []) then {
			...
			{
				[_varName,[_mode,_params,_functionName,_x,_isPersistent,_isCall,_callerName,_callerUID]] call noaim_fnc_MPexec;
			} foreach _target;
		} else {
			...
			noaim_fnc_MP_packet = [1,_params,_functionName,_target,_isPersistent,_isCall,"__SERVER__","__SERVER__"];

			if (_ownerID < 0) then {
				publicvariable "noaim_fnc_MP_packet";
			} else {
				if (_ownerID != _serverID) then {
					_ownerID publicvariableclient "noaim_fnc_MP_packet";
				};
			};

			if (_ownerID < 0 || _ownerID == _serverID) then {
				["noaim_fnc_MP_packet",noaim_fnc_MP_packet] spawn noaim_fnc_MPexec;
			};
