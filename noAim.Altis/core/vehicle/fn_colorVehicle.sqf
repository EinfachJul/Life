...
_onlyLocal = [_this,2,false,[false]] call BIS_fnc_param;
_loopcount = 0;

while {_loopcount < 10} do
{
...
if(local _vehicle && !_onlyLocal) then 
{
...
_texture = [(typeOf _vehicle)] call noaim_fnc_vehicleColorCfg;
...
if(local _vehicle) then
{
	if (_onlyLocal) then
	{
		_vehicle setVariable["Life_VEH_color",_index];
	}
	else
	{
		_vehicle setVariable["Life_VEH_color",_index,true];
		waitUntil{sleep 0.1; !isNil {_vehicle getVariable "Life_VEH_color"}};
	};
};
