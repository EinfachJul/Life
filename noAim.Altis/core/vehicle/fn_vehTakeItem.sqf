...
_num = [_ctrl,_num,life_carryWeight,life_maxWeight] call noaim_fnc_calWeightDiff;
if(_num == 0) exitWith {hint localize "STR_NOTF_InvFull"};
_weight = ([_ctrl] call noaim_fnc_itemWeight) * _num;
if(_ctrl == "money") then
{
	if(_num == _value) then
	{
...
{
	if([true,_ctrl,_num] call noaim_fnc_handleInv) then
	{
		if(_num == _value) then
		{
			_data set[_index,-1];
...
//[[life_trunk_vehicle],"TON_fnc_vehicleSaveItems",false] spawn noaim_fnc_TRY; //By Nukefliege
