...
_action = [
	format[(localize "STR_GNOTF_MaxMemberMSG")+ "<br/><br/>" +(localize "STR_GNOTF_CurrentMax")+ "<br/>" +(localize "STR_GNOTF_UpgradeMax")+ "<br/>" +(localize "STR_GNOTF_Price")+ " <t color='#8cff9b'>€%3</t>",_maxMembers,_slotUpgrade,[_upgradePrice] call noaim_fnc_numberText],
...
if(_action) then {
	if((grpPlayer getVariable "gang_bank") < _upgradePrice) exitWith {
		hint parseText format[
			...
			[_funds] call noaim_fnc_numberText,
			[_upgradePrice - _funds] call noaim_fnc_numberText
		];
	};
	__SUB__(_funds,_upgradePrice);
	grpPlayer setVariable["gang_maxMembers",_slotUpgrade,true];
	hint parseText format[localize "STR_GNOTF_UpgradeSuccess",_maxMembers,_slotUpgrade,[_upgradePrice] call noaim_fnc_numberText];
	//[[2,grpPlayer],"TON_fnc_updateGang",false,false] spawn noaim_fnc_DONE;
