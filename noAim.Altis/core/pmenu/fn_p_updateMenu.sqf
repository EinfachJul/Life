...
_side = switch(playerSide) do {case west:{"cop"}; case civilian:{"civ"}; case east:{"med"}; case independent:{"alac"};};
...
_mstatus ctrlSetStructuredText parseText format["<img size='1.3' image='icons\bank.paa'/> <t size='0.8px'>€%1</t><br/><img size='1.2' image='icons\money.paa'/> <t size='0.8'>€%2</t>",[life_zgazzy] call noaim_fnc_numberText,[life_znorak] call noaim_fnc_numberText];
ctrlSetText[2009,format["Weight: %1 / %2", life_carryWeight, life_maxWeight]];
{
	_str = [_x] call noaim_fnc_varToStr;
	_shrt = [_x,1] call noaim_fnc_varHandle;
...
} foreach zoki_inv_items;
{
	if((_x select 1) == _side) then
	{
		_str = [_x select 0] call noaim_fnc_varToStr;
