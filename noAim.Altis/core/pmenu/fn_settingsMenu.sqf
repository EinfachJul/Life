...
{
	if(!createDialog "SettingsMenu") exitWith {hint "Menu fehlerhaft, das menu oeffnet sich nicht?"};
};
...
if(life_revealObjects) then {
	...
	_objs buttonSetAction "[LIFE_ID_RevealObjects,""onEachFrame""] call BIS_fnc_removeStackedEventHandler; life_revealObjects = false; [] call noaim_fnc_settingsMenu;";
} else {
	_objs ctrlSetTextColor [1,0,0,1];
	_objs ctrlSetText "OFF";
	_objs buttonSetAction "LIFE_ID_RevealObjects = [""LIFE_RevealObjects"",""onEachFrame"",""noaim_fnc_revealObjects""] call BIS_fnc_addStackedEventHandler; life_revealObjects = true; [] call noaim_fnc_settingsMenu;";
};

if(life_tagson) then {
	_tags ctrlSetTextColor [0,1,0,1];
	_tags ctrlSetText "ON";
	_tags buttonSetAction "[LIFE_ID_PlayerTags,""onEachFrame""] call BIS_fnc_removeStackedEventHandler; life_tagson = false; [] call noaim_fnc_settingsMenu;";
} else {
	_tags ctrlSetTextColor [1,0,0,1];
	_tags ctrlSetText "OFF";
	_tags buttonSetAction "LIFE_ID_PlayerTags = [""LIFE_PlayerTags"",""onEachFrame"",""noaim_fnc_playerTags""] call BIS_fnc_addStackedEventHandler; life_tagson = true; [] call noaim_fnc_settingsMenu;";
};
