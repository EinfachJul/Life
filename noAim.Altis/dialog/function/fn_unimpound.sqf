...
if(life_zgazzy < _price && life_znorak < _price) exitWith {hint format[(localize "STR_Garage_CashError"),[_price] call noaim_fnc_numberText];};
...
if(typeName life_garage_sp == "ARRAY") then {
	...
	//[[_vid,_pid,life_garage_sp select 0,_unit,_price,life_garage_sp select 1],"TON_fnc_spawnVehicle",false,false] spawn noaim_fnc_DONE;
	[_vid,_pid,life_garage_sp,_unit,_price] remoteExec ["TON_fnc_spawnVehicle",2];
} else {
	if(life_garage_sp in ["medic_spawn_1","medic_spawn_2","medic_spawn_3"]) then {
		[_vid,_pid,life_garage_sp,_unit,_price] remoteExec ["TON_fnc_spawnVehicle",2];
		//[[_vid,_pid,life_garage_sp,_unit,_price],"TON_fnc_spawnVehicle",false,false] spawn noaim_fnc_DONE;
	} else {
...
[format ["UNIMPOUND: %1 hat %2 ausgeparkt.", name player, _vehicle],"ax_log",false,false] call noaim_fnc_Loggy;